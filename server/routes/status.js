...
<div class="status-shell">
  ...
  <a class="brand" href="/" aria-label="Capital Flow home"><img src="/logo-gold.jpeg" alt="Capital Flow logo"><div><strong>CAPITAL FLOW</strong><span>System status</span></div></a>
    <div class="nav-actions"><a class="nav-link" href="/status">Public status</a><a class="nav-link" href="/status/admin">Operations</a></div>
  </header>
  <main>
    <section class="hero" aria-labelledby="overall-title">
      ...
      <div class="hero-status"><div id="overall-badge" class="status-dot unknown" role="status" aria-live="polite">Checking</div><div class="last-check" id="last-check">Last check —</div><div class="last-check" id="next-check">Next automatic check —</div></div>
    </section>
    ...
    <div class="admin-area" id="admin-area">
      <section class="section"><div class="section-head"><h2>Operations console</h2><p>Private monitoring controls</p></div><div class="card admin-panel"><h3>Admin authentication</h3><p>Use the existing admin token or sign in as the configured administrator. The token is stored locally in this browser only and is sent in a request header.</p><div class="admin-auth"><input class="admin-input" id="admin-token" type="password" autocomplete="off" placeholder="Static admin token"><button class="admin-button" id="save-token" type="button">Use token</button><button class="admin-button secondary" id="check-now" type="button">Run check now</button><a class="nav-link" href="${getFullAdminUrl()}">Full user admin</a></div><div class="admin-help" id="admin-auth-status"></div></div></section>
...
(function(){
  ...
  byId('refresh-admin').addEventListener('click',function(){loadAdmin();loadSummary();toast('Diagnostics refreshed')});
  ...
  loadSummary();loadAdmin();setInterval(loadSummary,60000);if(ADMIN_PAGE)setInterval(loadAdmin,60000);
})();
</script>
</body>
</html>`;
