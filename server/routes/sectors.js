...
router.get('/sector-flow', requireScanQuota('sectorMoving'), async (req, res) => {
  if (flowCache.results && flowCache.expiresAt > Date.now()) {
    ...
    return res.json({ results: flowCache.results, fetchTime: flowCache.fetchTime, fromCache: true, ...quotaFor(req.user) });
  }
  ...
  try {
    const results = await Promise.all(etfs.map(async (symbol) => {
      try {
        ...
        const recent = quotes
          .filter(function(d) { return d.volume && d.volume > 0; })
          .sort(function(a, b) { return new Date(b.date) - new Date(a.date); })
          .slice(0, 10);
        const avgVol = recent.length >= 3 ? Math.round(recent.reduce(function(s, d) { return s + d.volume; }, 0) / recent.length) : 0;
        ...
        return { symbol: symbol, price: 0, change: 0, volume: 0, avgVolume: 0, volRatio: 0, flow: 'neutral', dayHigh: 0, dayLow: 0, prevClose: 0, lastSession: false };
      }
    }));
    spendScan(req.user, 'sectorMoving');
