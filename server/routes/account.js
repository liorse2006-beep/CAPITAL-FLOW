...
router.get('/account/export', requireAuth, async (req, res) => {
  try {
    ...
    const [user, watchlist, alerts, schedules, radars, notifications, chatMessages, feedback] = await Promise.all([
      ...
      db
        .prepare(
          'SELECT id, name, mode, selected_sectors_json, min_volume_ratio, min_market_cap, min_volume, min_price, max_price, schedule_time_1, schedule_time_2, expires_on, active, last_check_at, last_success_at, last_partial_count, created_at, updated_at FROM capital_flow_radars WHERE user_id = ? ORDER BY created_at ASC'
        )
        .all(userId),
      db
        .prepare(
