...
router.delete('/watchlist-alerts/:symbol',  requireElite, (req, res) => { removeAlert(req.user.id, req.params.symbol.toUpperCase()); res.json({ ok: true }); });
router.delete('/watchlist-alerts',          requireElite, (req, res) => { clearAlerts(req.user.id); res.json({ ok: true }); });
