...
const db = require('../db');
const { hashPassword, verifyPassword, issueToken, withEffectivePremium, generateOTP, saveOTP, verifyOTP } = require('../services/auth');
...
const { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_CALLBACK_URL, HCAPTCHA_SECRET, FRONTEND_URL, ADMIN_EMAIL } = require('../config');
  ...
  passport.use(new GoogleStrategy(
    ...
    function(accessToken, refreshToken, profile, done) {
      ...
      if (!user) {
        ...
        if (user) {
          db.prepare('UPDATE users SET google_id = ?, google_email = ?, is_verified = 1 WHERE id = ?')
...
router.post('/signup', authLimiter, async (req, res) => {
  try {
    ...
    if (existing && existing.is_verified) return res.status(409).json({ error: 'An account with this email already exists' });
...
router.post('/verify-otp', otpLimiter, (req, res) => {
  ...
  res.json({ success: true, token, user: { id: user.id, email: user.email, is_premium: user.is_premium, is_pilot: !!user.is_pilot, tier: user.tier || 'free' } });
});
...
router.post('/accept-pilot-terms', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE users SET pilot_terms_accepted_at = ? WHERE id = ?')
