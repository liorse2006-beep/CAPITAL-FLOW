...
describe('AlertThresholdModal', () => {
  it('shows a type picker with the ticker symbol when there is no existing alert', () => {
    render(<AlertThresholdModal symbol="AAPL" current={null} currentPrice={190} onSave={vi.fn()} onRemove={vi.fn()} onClose={vi.fn()} />);
  ...
  it('calls onSave with a volume alert object when set from the type picker', async () => {
    ...
    const onSave = vi.fn();
    render(<AlertThresholdModal symbol="AAPL" current={null} currentPrice={190} onSave={onSave} onRemove={vi.fn()} onClose={vi.fn()} />);
  ...
  it('calls onSave with a price alert object, including the computed starting side', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'קבע התראה' }));
    expect(onSave).toHaveBeenCalledWith({ type: 'price', targetPrice: 200, referencePrice: 190, startingSide: 'below' });
  });
  ...
  it('closes on Escape', () => {
    const onClose = vi.fn();
    render(<AlertThresholdModal symbol="AAPL" current={null} currentPrice={190} onSave={vi.fn()} onRemove={vi.fn()} onClose={onClose} />);
  ...
  it('closes when clicking the backdrop', async () => {
    ...
    const { container } = render(
      <AlertThresholdModal symbol="AAPL" current={null} currentPrice={190} onSave={vi.fn()} onRemove={vi.fn()} onClose={onClose} />
    );
