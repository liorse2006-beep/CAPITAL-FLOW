...
const TICKER_POOL = [
  ...
  'JPM','V','XOM','UNH','MA','COST','HD','PG','JNJ','MRK',
  'ABBV','CVX','WMT','KO','PEP','BAC','CRM','ADBE','AMD','NFLX',
  'TMO','ORCL','ACN','MCD','LIN','ABT','DIS','WFC','CSCO','DHR',
  'INTC','VZ','TXN','PM','NKE','CMCSA','NEE','QCOM','UPS','RTX',
  'HON','UNP','LOW','AMGN','IBM','SPGI','INTU','CAT','GE','BA',
  'DE','ELV','PLD','AMAT','SBUX','MDT','GS','BLK','ISRG','ADI',
  'BKNG','GILD','MMC','SYK','TJX','LRCX','VRTX','MDLZ','ADP','C',
  'CVS','REGN','CI','ZTS','PGR','SO','MO','BSX','FISV','DUK',
  'CB','SCHW','TGT','SLB','EOG','APD','AON','ITW','BDX','CL',
  'MU','SHW','PANW','SNPS','CDNS','KLAC','MRVL','ORLY','MCK','NOC',
  'HUM','ROP','FCX','PYPL','MPC','PSX','VLO','WM','EMR','ETN',
  'CSX','PXD','AJG','MSI','GD','APH','ADSK','ANET','NSC','PCAR',
  'AZO','TRV','COF','MET','AIG','SPG','O','DOW','CTAS','ECL',
  'CMG','FTNT','ROST','KMB','GIS','HLT','MAR','YUM','DXCM','IDXX',
  'EA','ODFL','CTSH','PAYX','VRSK','FAST','EXC','AEP','XEL','ED',
  'PEG','WEC','ES','FE','AWK','DTE','PPL','CMS','ATO','NI',
  'SQ','SHOP','UBER','ABNB','DASH','SNOW','PLTR','RBLX','COIN','RIVN',
  'LCID','SOFI','HOOD','MRNA','BNTX','ZM','DOCU','TWLO','NET','DDOG',
]
...
export default function ScanLoader({ label, matches, statusMessages }) {
  ...
  var messages = statusMessages && statusMessages.length ? statusMessages : [
  ...
  useEffect(function() {
    var interval = setInterval(function() {
      setStatusIdx(function(i) { return (i + 1) % messages.length })
    }, 2200)
    return function() { clearInterval(interval) }
  }, [messages.length])
  ...
  useEffect(function() {
    if (matches != null && matches !== prevMatches.current) {
      ...
      setPulse(true)
      var t = setTimeout(function() { setPulse(false) }, 220)
      return function() { clearTimeout(t) }
    }
  }, [matches])

  return React.createElement('div', { className: 'scan-loader' },
    React.createElement('div', { className: 'scan-loader-top' },
      React.createElement('span', { className: 'scan-loader-label' }, label),
      React.createElement('span', { className: 'scan-loader-dot' })
    ),
    React.createElement('div', { className: 'scan-loader-tape' },
      React.createElement('div', { className: 'scan-loader-tape-track' },
    ...
    matches != null && React.createElement('div', { className: 'scan-loader-matches' },
      React.createElement('span', { className: 'scan-loader-matches-num' + (pulse ? ' pulse' : '') }, matches),
      React.createElement('span', { className: 'scan-loader-matches-label' }, matches === 1 ? 'match found' : 'matches found')
    ),
