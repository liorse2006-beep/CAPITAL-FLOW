...
export default function MoneyFlow({ theme, setShowUpgradeModal, onSignIn }) {
  ...
  const fetchFlow = useCallback(function() {
    ...
    setLoading(true);
    setError(null);
    fetch("/api/sector-flow", { headers: { Authorization: 'Bearer ' + getToken() } }).then(function(r) {
      if (r.status === 401) { onSignIn(); throw new Error("Sign in to run a scan"); }
      if (r.status === 403) return r.json().then(function(d) { throw Object.assign(new Error(d.error || "Limit reached"), { code: d.code }); });
      if (!r.ok) return r.json().then(function(d) { throw new Error(d.error || "Fetch failed"); });
      return r.json();
    }).then(function(d) {
  ...
  const inflows = flowData ? flowData.filter(function(d) { return d.flow === "inflow"; }).sort(function(a, b) { return b.volRatio - a.volRatio; }) : [];
  const outflows = flowData ? flowData.filter(function(d) { return d.flow === "outflow"; }).sort(function(a, b) { return b.volRatio - a.volRatio; }) : [];
  const neutrals = flowData ? flowData.filter(function(d) { return d.flow === "neutral"; }) : [];
  const etfMap = {};
  SECTOR_ETFS.forEach(function(s) { etfMap[s.ticker] = s; });
  ...
  return (
    <div className="page-content">
      <div className="flow-header">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="scan-btn" onClick={fetchFlow} disabled={loading}>
            {loading ? React.createElement(React.Fragment, null, React.createElement("div", { className: "spinner" }), " Fetching...") : "Refresh Flow"}
          </button>
        </div>
      </div>

      {!isPremium && scanMeta && (function() {
        var q = categoryQuota(scanMeta, "sectorMoving");
        var pct = scanMeta.tier === "premium" ? ((q.used || 0) / (q.limit || 5) * 100) : (q.exhausted ? 100 : 0);
        return React.createElement('div', { className: 'ma-usage', style: { marginBottom: 16 } },
      ...
      {flowData && React.createElement("div", { style: { marginBottom: 16 } },
        React.createElement(SectorHeatmap, {
          flowData: flowData,
          etfMap: etfMap,
          onSectorClick: function(cell) { setExpandedETF(expandedETF === cell.symbol ? null : cell.symbol); }
        })
      )}
      ...
      {flowData && (function() {
        ...
        var sorted = [].concat(flowData).sort(function(a, b) {
          ...
          else if (flowSort === "sector") { av = (etfMap[a.symbol] && etfMap[a.symbol].name) || a.symbol; bv = (etfMap[b.symbol] && etfMap[b.symbol].name) || b.symbol; }
        ...
        var FTH = function(props) {
          return (
            <th className={flowSort === props.field ? "active" : ""} onClick={function() { handleFlowSort(props.field); }}>
              {props.label}{flowSort === props.field && <span className="sort-icon">{flowSortDir === "asc" ? "▲" : "▼"}</span>}
            </th>
          );
        };
        return (
              ...
              <table>
                ...
                <tbody>
                  {sorted.map(function(d, i) {
                    ...
                    return (
                      React.createElement(React.Fragment, { key: d.symbol },
                        React.createElement("tr", { className: "flow-row " + (open ? "expanded" : ""), onClick: function() { setExpandedETF(open ? null : d.symbol); } },
                          React.createElement("td", { className: "col-rank" }, i + 1),
                          React.createElement("td", { className: "col-ticker" }, d.symbol),
                          React.createElement("td", { className: "col-name", style: { fontFamily: "var(--font)" } }, (etf && etf.name) || d.symbol),
                          React.createElement("td", null, "$" + d.price.toFixed(2)),
                          React.createElement("td", { className: d.change >= 0 ? "col-pos" : "col-neg" },
                            (d.change >= 0 ? "+" : "") + d.change + "%"
                          ),
                          React.createElement("td", null,
                            React.createElement("span", { className: "ratio-pill " + (d.volRatio >= 2 ? "hot" : d.volRatio >= 1.2 ? "warm" : "ok") }, d.volRatio + "x")
                          ),
                          React.createElement("td", { style: { color: "var(--text-2)", fontFamily: "var(--mono)", fontSize: 12 } }, fmt(d.volume)),
                          React.createElement("td", null, React.createElement("span", { className: "flow-badge " + d.flow }, d.flow.toUpperCase()))
                        ),
                        open && etf && etf.holdings && React.createElement("tr", { className: "holdings-row" },
                          React.createElement("td", { colSpan: "8" },
                            React.createElement("div", { className: "holdings-inline" },
                              React.createElement("span", { className: "holdings-title" }, "Top Holdings"),
                              React.createElement("div", { className: "holdings-chips" },
                                etf.holdings.map(function(h) {
                                  return React.createElement("div", { key: h.sym, className: "holding-chip" },
