...
export default function AuthModal({ onClose }) {
  ...
  const [screen, setScreen] = useState(
    pendingGoogleToken ? 'google_confirm' : 'login'
  ) // login | signup | otp | forgot | reset | google_confirm
  ...
  function switchScreen(s) {
    setScreen(s); setError(''); setPassword(''); setOtp(''); setCaptchaToken('')
    captchaRef.current?.resetCaptcha()
  }
      ...
      <div className="auth-panel">
        ...
        {(screen === 'login' || screen === 'signup') && (
          <>
            <div className="auth-tabs">
              <button className={`auth-tab ${screen === 'login' ? 'active' : ''}`} onClick={() => switchScreen('login')}>Log In</button>
              <button className={`auth-tab ${screen === 'signup' ? 'active' : ''}`} onClick={() => switchScreen('signup')}>Sign Up</button>
            </div>
        ...
        {screen === 'google_confirm' && pendingGoogleToken && (
          <div className="auth-google-consent">
            ...
            <h2 className="auth-otp-title" style={{ marginTop: 12 }}>Continue as</h2>
            ...
            <div className="auth-consent-actions">
              <button className="auth-consent-cancel" onClick={() => { cancelGoogleLogin(); onClose() }}>Cancel</button>
              <button className="auth-consent-allow" onClick={() => { confirmGoogleLogin(); onClose() }}>
                Allow
