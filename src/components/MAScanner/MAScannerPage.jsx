...
const ALL_SECTORS      = [
  ...
  'Energy','Industrials','Materials','Real Estate','Utilities','Communication Services','Semiconductors',
];
...
export default function MAScannerPage({ onOpenChart, onSignIn }) {
  ...
  function startScan() {
    ...
    fetch(`/api/scan-ma?${params}`, { headers: authH() })
      .then(r => {
        if (r.status === 403) return r.json().then(d => { throw Object.assign(new Error(d.error || 'Limit reached'), { code: d.code }); });
        if (!r.ok) return r.json().then(d => { throw new Error(d.error || 'Scan failed'); });
        return r.json();
      })
      .then(d => {
    ...
    pollRef.current = setInterval(() => {
      fetch('/api/ma-progress', { headers: authH() })
        .then(r => r.json())
        .then(d => { if (d.running) setProgress(d); })
        .catch(() => {});
    }, 1500);
  }
  ...
  const filtered = (results || []).filter(r => {
    ...
    return r.symbol.includes(search.toUpperCase()) ||
  ...
  return React.createElement('div', { className: 'page-content' },
      ...
      React.createElement('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } },
        ...
        !isLocked && React.createElement('button', {
          ...
          loading
            ...
            : (results ? 'Re-scan' : 'Run MA Scan')
        )
      )
    ),

    // ── Sign-in banner ───────────────────────────────────────────────────────
    isLocked && React.createElement('div', { className: 'ma-signin-banner' },
      React.createElement('div', null,
        React.createElement('div', { className: 'ma-signin-title' }, '🔒 Sign in to unlock MA Scanner'),
        React.createElement('div', { className: 'ma-signin-sub' }, 'Free account includes 3 scans. Upgrade to Premium for unlimited.')
      ),
      React.createElement('button', { className: 'scan-btn', style: { flexShrink: 0 }, onClick: onSignIn }, 'Sign In / Register')
    ),
    ...
    React.createElement('div', {
      ...
      // Moving Average
      React.createElement('div', { className: 'ma-ctrl-group' },
        React.createElement('span', { className: 'ma-ctrl-label' }, 'Moving Average'),
        React.createElement('div', { className: 'ma-ctrl-btns' },
          MA_OPTIONS.map(v =>
      ...
      React.createElement('div', { className: 'ma-ctrl-group' },
        ...
        React.createElement('div', { className: 'ma-ctrl-btns' },
          React.createElement('button', {
            ...
            onClick: () => setInterval('1d'), disabled: loading,
          }, 'Daily'),
          React.createElement('button', {
            className: 'ma-ctrl-btn' + (interval === '1wk' ? ' active' : ''),
            onClick: () => setInterval('1wk'), disabled: loading,
          }, 'Weekly')
        )
      ),
      ...
      // Market
      React.createElement('div', { className: 'ma-ctrl-group ma-ctrl-group-wide' },
        ...
        React.createElement('div', { className: 'ma-ctrl-btns' },
          MARKET_OPTIONS.map(opt =>
            React.createElement('button', {
              ...
              onClick: () => { setMarket(opt.key); if (opt.key !== 'sectors') setSelectedSectors([]); },
              disabled: loading,
            },
      ...
      // Usage counter
      !isPremium && scanMeta && React.createElement('div', { className: 'ma-usage' },
        React.createElement('div', { className: 'ma-usage-bar-wrap' },
          React.createElement('div', {
            className: 'ma-usage-bar',
            style: { width: (scanMeta.tier === 'premium' ? ((scanMeta.premium ? scanMeta.premium.used : 0) / 5 * 100) : (categoryQuota(scanMeta, 'maScanner').exhausted ? 100 : 0)) + '%' },
          })
        ),
    ...
    !isLocked && market === 'sectors' && React.createElement('div', { className: 'ma-sector-grid-wrap' },
      React.createElement('div', { className: 'sector-grid-header' },
        React.createElement('span', null, 'Select Sectors'),
        selectedSectors.length > 0 && React.createElement('button', {
      ...
      selectedSectors.length === 0 && React.createElement('div', { className: 'sector-hint' },
        'No sectors selected — will scan top 5 from all sectors'
      ),
      !isPremium && React.createElement('div', { className: 'sector-hint' },
        'Free tier: up to ' + MAX_FREE_SECTORS + ' sectors. Upgrade for unlimited.'
      )
    ),

    // ── Scan limit banner ────────────────────────────────────────────────────
    scanLimitReached && React.createElement('div', { className: 'ma-limit-banner' },
      React.createElement('div', { className: 'ma-limit-icon' }, '🔒'),
      React.createElement('div', null,
        React.createElement('div', { className: 'ma-limit-title' }, 'Scan limit reached'),
        React.createElement('div', { className: 'ma-limit-sub' },
    ...
    // ── Scan loader ──────────────────────────────────────────────────────────
    loading && progress && React.createElement(ScanLoader, {
    ...
    error && React.createElement('div', { className: 'error-bar error-bar-action' },
      React.createElement('div', { className: 'error-bar-content' }, React.createElement('span', null, friendlyError(error))),
      React.createElement('div', { className: 'error-bar-actions' },
        React.createElement('button', { className: 'error-retry-btn', onClick: startScan, disabled: scanLimitReached }, 'Retry'),
        React.createElement('button', { className: 'error-dismiss-btn', onClick: () => setError(null) }, 'Dismiss')
      )
    ),

    // ── Empty state ──────────────────────────────────────────────────────────
    !loading && !results && !error && !scanLimitReached &&
      React.createElement('div', { className: 'empty' },
        ...
        !isPremium && scanMeta && React.createElement('p', { style: { color: 'var(--text-2)', fontSize: 13, marginTop: 8 } },
    ...
    results && React.createElement(React.Fragment, null,
      React.createElement('div', { className: 'table-card' },
        ...
        React.createElement('div', { className: 'mobile-cards' },
          sorted.length === 0
            ? React.createElement('div', { className: 'empty', style: { padding: '32px 20px' } },
            ...
            : sorted.map((r, i) => {
                ...
                return React.createElement('div', { key: r.symbol, className: 'mobile-card' },
                  ...
                  React.createElement('div', { className: 'mobile-card-mid' },
                    React.createElement('span', { className: 'mobile-card-price' }, r.price ? '$' + r.price.toFixed(2) : '—'),
                    React.createElement('span', { className: 'mobile-card-change', style: { color: distColor } },
                      (r.maDistance >= 0 ? '+' : '') + r.maDistance.toFixed(2) + '%'
                    )
                  ),
                  React.createElement('div', { className: 'mobile-card-bottom' },
                    React.createElement('span', { className: 'mobile-card-vol' }, `SMA${ma}: $${r.maValue.toFixed(2)}`)
                  )
                );
              })
        ),
        ...
        React.createElement('div', { className: 'table-wrap' },
          React.createElement('table', null,
            ...
            React.createElement('tbody', null,
              sorted.length === 0
                ? React.createElement('tr', null,
                    React.createElement('td', { colSpan: 6, style: { textAlign: 'center', padding: '32px', color: 'var(--text-2)', fontFamily: 'var(--mono)', fontSize: 12 } },
                ...
                : sorted.map((r, i) => {
                    ...
                    const distAbs  = Math.abs(r.maDistance);
                    const distColor = distAbs < 0.5 ? 'var(--accent)' : isAbove ? 'var(--green)' : 'var(--red)';
                    return React.createElement('tr', { key: r.symbol, className: 'flow-row' },
                      ...
                      React.createElement('td', null,
                        React.createElement('span', { className: 'ma-distance-cell', style: { color: distColor } },
                          (r.maDistance >= 0 ? '+' : '') + r.maDistance.toFixed(2) + '%'
                        )
                      )
                    );
